...
export default function ScanRecord() {
  ...
  const onStart = async () => {
    ...
    try {
      ...
      const video = await cameraRef.current.recordAsync({ maxDuration: total });
      ...
      router.replace({
        pathname: "/(onboarding)/scan-analyzing" as any,
  ...
  const canStart = permission.granted && cameraReady && !running && !starting;
        ...
        {permission.granted ? (
          ...
          <View style={styles.previewFallback}>
            <Text style={{ color: subtext, textAlign: "center" }}>
              Camera access is required to record your scan.
            </Text>
            <Pressable
...
const styles = StyleSheet.create({
  container: { flex: 1, padding: 16, paddingTop: 50, gap: 12 },
